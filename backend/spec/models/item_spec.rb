...
RSpec.describe Item, type: :model do
  context 'validations' do
    ...
    it "is invalid without a description" do
      item = Item.new(name: "Kitten", number: 1, status: "Available")
      expect(item).not_to be_valid
    end
    ...
    it "is invalid without a status" do
      item = Item.new(name: "Kitten", description: "A fluffy kitten", number: 1)
      expect(item).not_to be_valid
    end

    it "is invalid with a status not in the allowed list" do
      item = Item.new(name: "Kitten", description: "A fluffy kitten", number: 1, status: "Lost")
      expect(item).not_to be_valid
    end
  end
end
